...
module.exports = function (app, passport) {
	...
	// 用户分类文章页
	pageRouter.get(
		"/u/:userName/category/:categoryName",
		pageCtrl.categoryArticles
	);
	// 用户文章按时间统计
	pageRouter.get(
		"/u/:userName/archive/:year/:month",
		pageCtrl.articleStatisticByDate
	);
	// 设置界面
	pageRouter.get(
		"/setting",
	...
	apiRouter.post(
		...
		requireSignIn,
		verifyLocalAccount,
		checkSchema(articleSchema.create),
		articleCtrl.saveArticle
	);
	// 获取 emoji 面板html
	...
	apiRouter.get(
		...
		articleCtrl.fetchArticleEditDetail
	);
	// 获取文章展示详情
	apiRouter.get(
		"/article/show/:articleId",
		articleCtrl.fetchArticleShowDetail
	);
	// 获取文章评论列表
	apiRouter.get(
		"/article/:articleId/comments",
		articleCtrl.queryArticleComments
	);
	// 点赞文章
	apiRouter.get(
		"/article/:articleId/heart",
		requireSignIn,
		articleCtrl.heartArticle
	);
	// 取消点赞文章
	apiRouter.delete(
		"/article/:articleId/heart",
		requireSignIn,
		articleCtrl.cancelHeartArticle
	);
	// 更新文章
	apiRouter.put(
		"/article",
		requireSignIn,
		verifyLocalAccount,
		checkSchema(articleSchema.update),
	...
	apiRouter.post(
		"/message/leave",
		requireSignIn,
		checkSchema(commentSchema.create),
		userCtrl.leaveMsg
	);
	// 获取留言列表
