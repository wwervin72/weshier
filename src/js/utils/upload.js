...
;(function ($, undefined) {
	$.fn.uploadfile = function (setting) {
		var defaultSetting = {
			...
			canDrag : true,
			canMultiple : true,
			imgPath: '/assets/static/image/upload/',
		...
		$(this).each(function (i, item) {
			...
			if(setting.canDrag){
				...
				demoHtml +=						'<label class="sel_file_img" title="点击选择文件">';
				demoHtml +=							'<span><img src="' + setting.imgPath + 'add_img.png"/></span>';
				...
				demoHtml +=						'<button class="uploadfile ws_btn">开始上传</button>';
				demoHtml +=					'</div>';
				demoHtml +=				'</div>';
				...
				Array.prototype.forEach.call(files, function (item, i) {
					...
					fr.onload = function () {
						...
						if(item.type.indexOf("image") > -1){
							...
							imgSrc = setting.imgPath + 'rar.png';
						}else if(item.name.indexOf("zip") > -1){
							imgSrc = setting.imgPath + 'zip.png';
						}else if(item.type.indexOf("text") > -1){
							imgSrc = setting.imgPath + 'txt.png';
						}else{
							imgSrc = setting.imgPath + 'file.png';
						}
