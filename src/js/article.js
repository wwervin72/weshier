		...
		if (res.status) {
			...
			fetchArticleCommentHtml(articleId, {
				...
				.then((res) => {
					...
					delegateCommentsEvent(
						comments,
						articleId,
						commentContainer,
						article.allow_comment === "1"
					);
				})
				.catch((err) =>
					message("留言列表加载失败，刷新试试看", "warning")
				);
		}
	});
}
...
const heartArticleStatistic =
	heartArticleBtn && heartArticleBtn.nextElementSibling;
if (heartArticleBtn) {
...
let s6 = document.querySelector(".ws_iconfont.ws_s_6");
if (s6) {
	s6.addEventListener("click", (evt) => {
