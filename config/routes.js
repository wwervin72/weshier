...
const { respond } = require('../app/utils')
...
module.exports = function (app, passport) {
	...
	// 编辑
	pageRouter.get('/write/:articleId', requireSignIn, verifyLocalAccount, pageCtrl.editor)
	// 用户主页
	pageRouter.get('/u/:userName', pageCtrl.userHomePage)
	// 用户实验室
	pageRouter.get('/practice/:userName', verifyLocalAccount, pageCtrl.practicePages)
	// 用户标签文章页
	pageRouter.get('/u/:userName/tag/:tagName', verifyLocalAccount, pageCtrl.tagArticles)
	// 用户分类文章页
	pageRouter.get('/u/:userName/category/:categoryName', verifyLocalAccount, pageCtrl.categoryArticles)
	// 用户文章按时间统计
	pageRouter.get('/u/:userName/archive/:year/:month', verifyLocalAccount, pageCtrl.articleStatisticByDate)
	// 设置界面
