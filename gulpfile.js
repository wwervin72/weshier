...
const bundleAssetsDir = argv.build_mode === 'deploy' && argv.assets_path ? argv.assets_path : './public/static/'
...
gulp.task('rev', () => {
	return gulp.src([path.join(jsAssetsDir, '*.js'), path.join(cssAssetsDir, '*.css')])
		.pipe(rev())
...
gulp.task('rev-collector', () => {
	return gulp.src([path.join(revAssetsDir, '*.json'), path.join(htmlAssetsDir, '*.html')])
		.pipe(revCollector({
			replaceReved: true
		}))
		.pipe(htmlmin({
...
gulp.task('clean', () => del([bundleAssetsDir, htmlAssetsDir], { force: true }))
