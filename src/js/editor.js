...
$(function () {
	...
	function save() {
		...
		const abstractLen = Math.floor(
			Math.random() * (abstractMax - abstractMin + 1) + abstractMin
		);
		const html = $(editor.getHTML());
		const allowCommentDom = document.querySelector(
			"input[name=allow_comment]:checked"
		);
	...
	editor = editormd("editor", {
		...
		onload() {
			...
			document.querySelector(".fa-switcher").parentNode.style.cssText +=
				"vertical-align:top;";
		},
	});
